...
task_name_list=("xcopa_zh") # "xcopa_zh" "xnli_hi" "gsm8k" "HumanEval"

for task_name in "${task_name_list[@]}"; do
    echo "current task: $task_name"


dataset_name_list=("dangerousqa") # ("BeaverTails" "catqa_english" "harmfulqa" "shadow-alignment" "dangerousqa")
...
safety_methods=dare
trained_model_path=../saved_models/other_baselines_peft/$safety_methods-$task_name
pretained_model_path=/home/yx/model_cache/WizardLM-7B-Uncensored
output_dir=../safety_results/other_baselines_peft/$safety_methods-$task_name-$dataset_name
